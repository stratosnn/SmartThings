 ...
 */
metadata {
	definition (name: "My Fibaro Dimmer 2 ZW5", namespace: "FibarGroup", author: "Fibar Group", runLocally: true, minHubCoreVersion: '000.025.0000', executeCommandsLocally: true, mnmn: "SmartThings", vid:"generic-dimmer-power-energy") {
		capability "Switch"
...
def updated() {
	...
	//encapSequence(processAssociations())
...
private parameterMap() {[
        ...
        [key: "allowedCmdsS1", num: 24, size: 1, type: "number", def: "0", min: 0, max: 31 , title: "Allowed command frames for S1 associations", descr: "Parameter determines, which actions will not result in sending frames to association groups. Values could be combined. \n 1 - do not send switching ON \n 2 - do not send switching OFF \n 4 - do not send changing dimming level \n 8 - don not send on double click \n 16 - send 0xFF value on double click"],
        [key: "allowedCmdsS2", num: 25, size: 1, type: "number", def: "0", min: 0, max: 31 , title: "Allowed command frames for S2 associations", descr: "Parameter determines, which actions will not result in sending frames to association groups. Values could be combined. \n 1 - do not send switching ON \n 2 - do not send switching OFF \n 4 - do not send changing dimming level \n 8 - don not send on double click \n 16 - send 0xFF value on double click"],
...
def setAssociationGroup(group, nodes, action, endpoint = null) {
    log.info "setAssociationGroup ${group}, nodes ${nodes}, action ${action}"
...
def processAssociations() {
   ...
   if (state.associationGroups) {
       ...
       if (true) log.info "${device.label?device.label:device.name}: Getting supported association groups from device"
       cmds <<  zwave.associationV2.associationGroupingsGet()
   }
   log.info "processAssociations groups ${associationGroups} state ${associationGroups}"
      ...
      if(state."actualAssociation${i}" != null){
         if(state."desiredAssociation${i}" != null || state."defaultG${i}") {
            ...
            ((state."desiredAssociation${i}"? state."desiredAssociation${i}" : [] + state."defaultG${i}") - state."actualAssociation${i}").each {
                if (it != null){
                    if (true) log.info "${device.label?device.label:device.name}: Adding node $it to group $i"
            ...
            ((state."actualAssociation${i}" - state."defaultG${i}") - state."desiredAssociation${i}").each {
                if (it != null){
                    if (true) log.info "${device.label?device.label:device.name}: Removing node $it from group $i"
                    cmds << zwave.associationV2.associationRemove(groupingIdentifier:i, nodeId:Integer.parseInt(it,16))
                    refreshGroup = true
                }
            }
            if (refreshGroup == true) cmds << zwave.associationV2.associationGet(groupingIdentifier:i)
            else if (true) log.info "${device.label?device.label:device.name}: There are no association actions to complete for group $i"
         }
      } else {
         if (true) log.info "${device.label?device.label:device.name}: Association info not known for group $i. Requesting info from device."
...
def zwaveEvent(physicalgraph.zwave.commands.associationv2.AssociationReport cmd) {
    if (true) log.debug "AssociationReport ${device.label?device.label:device.name}: ${cmd}"
    def temp = []
    if (cmd.nodeId != []) {
       cmd.nodeId.each {
    ...
    if (true) log.info "${device.label?device.label:device.name}: Associations for Group ${cmd.groupingIdentifier}: ${temp}"
    updateDataValue("associationGroup${cmd.groupingIdentifier}", "$temp")
}

def zwaveEvent(physicalgraph.zwave.commands.associationv2.AssociationGroupingsReport cmd) {
    if (true) log.debug "AssociationGroupingsReport ${device.label?device.label:device.name}: ${cmd}"
    //sendEvent(name: "groups", value: cmd.supportedGroupings)
    if (true) log.info "AssociationGroupingsReport ${device.label?device.label:device.name}: Supported association groups: ${cmd.supportedGroupings}"
